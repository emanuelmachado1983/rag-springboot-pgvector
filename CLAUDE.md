...
- **Servidor MCP** (`spring-ai-starter-mcp-server-webmvc`, versión 1.1.8 de Spring AI) corriendo en el mismo proceso y puerto que el REST, transporte HTTP/SSE (`GET /sse` + `POST /mcp/message?sessionId=...`). Expone dos tools: `buscar_documentos_relevantes` (reutiliza directo `EmbeddingService` y `DocumentChunkRepository`, sin lógica duplicada) y `resumir_chunks` (combina una lista de fragmentos en un resumen único vía Claude). Se eligió SSE en vez de stdio para no tener que correr un segundo proceso separado del Spring Boot ya existente. Importante: Spring AI 2.0.0 usa Jackson 3 y rompe contra este stack (Spring Boot 3.5.x / Jackson 2) — por eso se fijó la versión en 1.1.8, no subir sin chequear esa incompatibilidad.
- **`resumir_chunks` (tool MCP, function calling encadenado)**: pensada para que el LLM orquestador la invoque por su cuenta después de `buscar_documentos_relevantes` cuando convenga sintetizar en vez de devolver chunks crudos. El criterio sugerido (en la descripción de ambas tools) es "más de 3 fragmentos". **Se probó forzar esta llamada desde el propio servidor** (que `DocumentSearchTool` invocara a `SummarizeChunksTool` directo cuando había más de 3 resultados, sin pasar por el LLM) porque en la práctica, con preguntas neutras, el LLM nunca llegaba a invocar `resumir_chunks` por su cuenta — pero se volvió atrás: la decisión de cuándo resumir queda 100% en manos del LLM orquestador, guiada solo por el texto de la tool, sin garantía de que la siga. Requiere `rag.llm.enabled=true`; si está apagado, devuelve un resultado con `fallback=true` en vez de fallar.
...
- `llm/SummarizeChunksTool.java` — tool MCP `resumir_chunks`, combina una lista de fragmentos en un resumen único vía Claude; fallback si el LLM está deshabilitado.
- `llm/ChunkInput.java` — entrada de `resumir_chunks` (documento + contenido), deliberadamente más chica que `SearchResult`.
- `llm/SummaryResult.java` — DTO de salida de `resumir_chunks` (mismo patrón fallback/error que `RagAnswerResponse`).
- `mcp/McpToolsConfig.java` — registra `DocumentSearchTool` y `SummarizeChunksTool` como `ToolCallbackProvider` para que el auto-config de Spring AI las exponga por MCP.
